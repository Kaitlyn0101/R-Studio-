...
Each cereal has a different serving size based on variables listed above which therefore could result in inaccurate data. Therefore, by using regression, we could normalize the data. 

```{r}
boxplot(sugars ~ shelf, xlab = "Shelf", ylab = "Sugar per serving (grams)")
abline(lm(y ~ x, cereal=cereal$shelf))
```
...
I am not sure as I created a regression line, but it does properlu fit where the data is. 
